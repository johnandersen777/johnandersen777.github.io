...
```bash
python pdf_to_markdown.py example.pdf output.md pdf_images/
```
"""
import fitz  # PyMuPDF
...
def extract_images_from_page(args):
    ...
    for img_index, img in enumerate(image_list, start=1):
        ...
        image_path = os.path.join(output_folder, f"page_{page_num+1}_img_{img_index}.{image_ext}")
...
async def convert_pdf_to_markdown(pdf_path, output_md_path, mode, executor=None, max_workers=1):
...
async def main(pdf_file, markdown_output, image_output_folder, mode, max_workers):
    if mode == 'serial':
        # Serial processing
        await convert_pdf_to_markdown(pdf_file, markdown_output, mode)
        extract_images_serial(pdf_file, image_output_folder)
    else:
        # Parallel processing
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            await convert_pdf_to_markdown(pdf_file, markdown_output, mode, executor, max_workers)
            await extract_images_parallel(pdf_file, image_output_folder, executor, max_workers)

if __name__ == "__main__":
    # Setup argparse to handle command-line arguments
    parser = argparse.ArgumentParser(description="Convert a PDF to Markdown and extract images.")
    parser.add_argument("pdf_file", type=str, help="The path to the PDF file to be converted.")
    parser.add_argument("markdown_output", type=str, help="The path where the output Markdown file will be saved.")
    parser.add_argument("image_output_folder", type=str, help="The folder where extracted images will be saved.")
    parser.add_argument("--mode", type=str, choices=['serial', 'parallel'], default='serial', help="Processing mode: 'serial' or 'parallel'.")
    parser.add_argument("--max-workers", type=int, default=4, help="Maximum number of worker processes (used in parallel mode).")

    args = parser.parse_args()

    # Run the main function asynchronously with parsed arguments
    asyncio.run(main(args.pdf_file, args.markdown_output, args.image_output_folder, args.mode, args.max_workers))
